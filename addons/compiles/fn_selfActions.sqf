...
if (_inVehicle && {_vehicle isKindOf "ArmoredSUV_Base_PMC"}) then {
   ...
   if (suv_close < 0) then {
     thesuv = _vehicle;
     suv_close = thesuv addAction ["Close Hatch","addons\animate\suv_close.sqf","",5,false,true];
     suv_open = thesuv addAction ["Open Hatch","addons\animate\suv_open.sqf","",5,false,true];
   };
} else {
...
if (_canDo && (speed player <= 1) && (_cursorTarget isKindOf "Plastic_Pole_EP1_DZ")) then {
    ...
    if(_owner == dayz_characterID || (getPlayerUID player) in _allowed)then{            
			s_player_plotManagement = player addAction ["Manage Plot", "addons\plotManagement\initPlotManagement.sqf", [], 5, false];
		};
	};
		  	s_player_maintain_area = player addAction [format[localize "STR_EPOCH_ACTIONS_MAINTAREA"], "addons\plotManagement\maintain_area.sqf", "maintain", 5, false];
		 	s_player_maintain_area_preview = player addAction [format[localize "STR_EPOCH_ACTIONS_MAINTPREV"], "addons\plotManagement\maintain_area.sqf", "preview", 5, false];
		 };
	} else {
...
if(_isModularDoor) then {
	...
	if (_player_studybody) then {
		if (s_player_studybody < 0) then {
				s_player_studybody = player addAction ["Check Wallet", "addons\actions\check_wallet.sqf",_cursorTarget, 0, false, true, "",""];
		};
	} else {
	...
	if (s_player_manageDoor < 0) then {		 
     s_player_manageDoor = player addAction ["Manage Door", "addons\doorManagement\initDoorManagement.sqf", _cursorTarget, 5, false];
	};
} else {
	...
	if(_typeOfCursorTarget in ZSC_MoneyStorage && (player distance _cursorTarget < 5)) then {
		if (s_bank_dialog < 0) then {
				s_bank_dialog = player addAction ["Money Storage", "addons\actions\bank_dialog.sqf",_cursorTarget, 3, true, true, "", ""];	
		};
	} else {
	...
	if( _isVehicle && !_isMan &&_isAlive && !_isMan && !locked _cursorTarget && !(_cursorTarget isKindOf "Bicycle") && (player distance _cursorTarget < 5)) then {		
		if (s_bank_dialog2 < 0) then {
			s_bank_dialog2 = player addAction ["Money Storage", "addons\actions\bank_dialog.sqf",_cursorTarget, 3, true, true, "", ""];
		};			
	} else {		
	...
	if (_isMan and _isAlive and !_isZombie and !_isAnimal and !(_traderType in serverTraders)) then {
		if (s_givemoney_dialog < 0) then {
			s_givemoney_dialog = player addAction [format["Give Money to %1", (name _cursorTarget)], "addons\actions\give_player_dialog.sqf",_cursorTarget, 3, true, true, "", ""];
		};
	} else {
	...
	if ((_cursorTarget isKindOf "ModularItems") || (_cursorTarget isKindOf "Land_DZE_WoodDoor_Base") || (_cursorTarget isKindOf "CinderWallDoor_DZ_Base")) then {
		...
		if (s_player_upgrade_build < 0) then {
			...
			s_player_upgrade_build = player addAction [format[localize "STR_EPOCH_ACTIONS_UPGRADE",_text], "addons\actions\player_upgrade.sqf",_cursorTarget, -1, false, true, "",""];
		};
	} else {
		player removeAction s_player_upgrade_build;
		s_player_upgrade_build = -1;
	...
	if((_isDestructable || _cursorTarget isKindOf "Land_DZE_WoodDoorLocked_Base" || _cursorTarget isKindOf "CinderWallDoorLocked_DZ_Base") && (DZE_Lock_Door == _ownerID)) then {
		...
		if (s_player_downgrade_build < 0) then {
			...
			s_player_downgrade_build = player addAction [format[localize "STR_EPOCH_ACTIONS_REMLOCK",_text], "addons\actions\player_buildingDowngrade.sqf",_cursorTarget, -2, false, true, "",""];
		};
	} else {
		player removeAction s_player_downgrade_build;
		s_player_downgrade_build = -1;
    ...
    if (_isMan and !_isAlive and !_isZombie and !_clothesTaken) then {
        if (s_player_clothes < 0) then {
            s_player_clothes = player addAction ["Take Clothes" , "addons\takeclothes\player_takeClothes_v2.sqf",[_cursorTarget], -10, false, true, "",""];
        };
    } else {
	...
	if (_typeOfCursorTarget in EVDVehicleArray and _hasToolbox and !(locked _cursorTarget) and (damage _cursorTarget < 1)) then {
		if (s_player_packvehicle < 0) then {
			s_player_packvehicle = player addAction ["Pack Vehicle", "addons\deploy_bike\EVD_pack.sqf",_cursorTarget, 0, false, true, "",""];
		};
	} else {
